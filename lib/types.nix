{ lib }: with lib; let
  ...
  in {
    ...
    in topLevel // optionalAttrs (options ? templateData) {
      templateData = mkIf (templateData != { }) (
        mapAttrs (_: mkDefault) templateData
      );
    };
  };
  schemeModules = [ schemeModule schemeDataModule aliasModule templateModule ];
  schemeType = types.submodule schemeModules;
in {
  inherit colourComponentModule colourComponentType
    colourModule colourType
    schemeModule schemeDataModule schemeModules schemeType
